...
write_csv(Cal_Poly,"/Volumes/GoogleDrive/My Drive/SCCOOS/CA_HAB_Bulletin/2018_11/Newport.csv")
...
#   select(location, date,`Pseudo-nitzschia_delicatissima_group_cells/L`,`Pseudo-nitzschia_seriata_group_cells/L`,`Chlorophyll_mg/m3`,`Domoic_Acid_ng/L`, Nitrate_uM, Phosphate_uM, Silicate_uM, latitude, longitude, year, month, day)
...
Santa_Monica_Nutrient_Plot = 
  ...
  theme(
    ...
    plot.title = element_text(hjust=0.5),
    title=element_text(size=12))
ggsave(file="Santa_Monica_Nutrient_Plot.png", plot=Santa_Monica_Nutrient_Plot, width=10, height=6)
...
#max(Cal_Poly_HAB$Pn_delicatissima) #25054.61
#max(Cal_Poly_HAB$Pn_seriata) #13859.99
#max(Cal_Poly_HAB$Alexandrium) # 31451.53
#max(Cal_Poly_HAB$Domoic_Acid) #NA

Cal_Poly_HAB_Plot = 
  ggplot(Cal_Poly_HAB, aes(x=Date))+
  geom_point(aes(y=Pn_delicatissima, color=Pn_delicatissima),size=3)+
  geom_point(aes(y=Pn_seriata,color=Pn_seriata),size=3)+
  scale_y_continuous(
    limits=c(0,300000),
...
Cal_Poly_DA_Plot = 
  ggplot(Cal_Poly_HAB, aes(x=date))+
  geom_point(aes(y=`Domoic_Acid_ng/L`, color="Domoic Acid (ng/L)"),size=3)+
  labs(title= "", x="Date", y="ng/L", color = "")+
...
Newport_HAB = SCCOOS_HAB_data %>% #44x8
  ...
  select(location,date,`Pseudo-nitzschia_delicatissima_group_cells/L`, `Pseudo-nitzschia_seriata_group_cells/L`,`Domoic_Acid_ng/L`)
...
#max(Newport_HAB$`Domoic_Acid_ng/L`) #
...
#max(Santa_Cruz_HAB$`Domoic_Acid_ng/L`) #0.83
...
#max(Santa_Monica_HAB$`Domoic_Acid_ng/L`) #Nan
...
#max(Stearns_Wharf_HAB$`Domoic_Acid_ng/L`) #Nan
