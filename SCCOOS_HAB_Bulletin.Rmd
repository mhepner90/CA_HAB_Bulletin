...
#Newport  
Newport_Org = read_csv("/Volumes/GoogleDrive/My Drive/SCCOOS/CA_HAB_Bulletin/2018_12/data/Newport_2018_12.csv") 

Newport = Newport_Org %>%
  mutate(Location = `Location`,
         Date = `Date`,
         Temperature = `Temp (C)`,
         Chlorophyll =`Avg Chloro`,
         #Nitrate = `Nitrate (uM)`,
         Silicate= `Silicate`,
         Phosphate = `Phosphate`,
         Pn_delicatissima = `Pseudo-nitzschia deli`,
         Pn_seriata = `Pseudo-nitzschia seriata`,
         Alexandrium = `Alexandrium spp.`,
         Domoic_Acid = `Domoic Acid`,
         Nitrate = `Nitrate`)%>%
  select(Location, Date, Temperature, Chlorophyll, Nitrate, Phosphate, Silicate, Pn_delicatissima, Pn_seriata, Alexandrium, Domoic_Acid)%>% 
  mutate(Year = format(as.Date(Date, format ="%Y-%m-%d"),"%Y"),
         Month = format(as.Date(Date, format ="%Y-%m-%d"),"%m"),
         Day = format(as.Date(Date, format ="%Y-%m-%d"),"%d"))
write_csv(Newport,"/Volumes/GoogleDrive/My Drive/SCCOOS/CA_HAB_Bulletin/2018_12/data/Newport_cleanedup.csv")
...
Santa_Monica = Santa_Monica_Org %>% 
    mutate(Location = `Location Code`,
         ...
         Phosphate = `Phosphate (uM)`,
         Pn_delicatissima = `Pseudo-nitzschia delicatissima`,
         Pn_seriata = `Pseudo-nitzschia seriata`,
         Alexandrium = `Alexandrium spp. (cells/L)`,
         Domoic_Acid = `Domoic Acid (ng/mL)`,
         Nitrate = `Nitrate (uM)`)%>%
