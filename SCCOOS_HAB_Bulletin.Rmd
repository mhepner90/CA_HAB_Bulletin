...
SCCOOS_HAB_data = SCCOOS_HAB_data %>%
  mutate(date = as.Date(paste(year, month, day, sep="-"))) %>% 
  mutate(date = format(as.Date(date, format ="%Y-%m-%d"),"%m-%d-%Y")) %>%
  mutate(`Domoic_Acid_ng/L` = (`Domoic_Acid_ng/mL`*1000)) %>% 
  #mutate(ymd = make_datetime(year,month,day))
  select(location, date,`Pseudo-nitzschia_delicatissima_group_cells/L`,`Pseudo-nitzschia_seriata_group_cells/L`,`Domoic_Acid_ng/L`, year, month, day)
...
#max(Cal_Poly_HAB$`Domoic_Acid_ng/L`) #NaN 
