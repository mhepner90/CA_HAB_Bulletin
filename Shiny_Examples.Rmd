...
Newport_Org = read_csv("/Volumes/GoogleDrive/My Drive/SCCOOS/CA_HAB_Bulletin/2019_05/HABMAP_data/NP_upload.csv")%>%
  mutate("Time Collected (hh:mm:ss PT)" = as.character("Time Collected (hh:mm:ss PT)"))
...
HAB_data = HAB_data %>%
  select('Location Code', 'Date Collected (YYYY-MM-DD)','pDA (ng/mL)','Pseudo-nitzschia delicatissima group (cells/L)','Pseudo-nitzschia seriata group (cells/L)','Alexandrium spp. (cells/L)','Temp (deg C)`,`Nitrate (uM)', 'Phosphate (uM)','Silicate (uM)','Ammonium (uM)','Avg Chloro (mg/m3)')

HAB_data_long = HAB_data %>%   
  gather(key = "Observations", value = "measurement", 'pDA (ng/mL)','Pseudo-nitzschia delicatissima group (cells/L)','Pseudo-nitzschia seriata group (cells/L)','Alexandrium spp. (cells/L)','Temp (deg C)`,`Nitrate (uM)', 'Phosphate (uM)','Silicate (uM)','Ammonium (uM)','Avg Chloro (mg/m3)')

write_rds(HAB_data_long, "HABMAP_Data/HAB_data_long.rds")
